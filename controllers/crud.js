...
exports.getMovies = async (req, res) => {
    const findMoviesOnly = await User.findById(req.user._id).select({_id: 0, movies: 1});
...
exports.saveMovie = async (req, res) => {
  ...
  try {
    ...
    if (movieDetails && user) {
      const matchesOne = user.movies.some(movie => movie.movieId === movieDetails.movieId)
      if (matchesOne) {
